...
assert(
    pkg.name === "bundle-size-analysis-skill",
    "package name must be bundle-size-analysis-skill"
);
assert(
    pkg.repository?.url ===
        "git+https://github.com/Nick2bad4u/Bundlephobia-Skill.git",
    "repository.url must exactly match the GitHub repository for npm trusted publishing"
);
assert(
    skill.path === "skills/bundle-size-analysis",
    "codexSkill.path must point at the nested skill directory"
);
